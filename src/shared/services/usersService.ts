...
class UsersService {
    ...
    return of(null).pipe(
      switchMap(async () => {
        try {
          ...
          // CONECTIVIDAD CON API: Crear usuario con contraseña
   ...
   * Conecta con la API y actualiza el estado reactivo
   */
  public updateUser(id: string, userData: Partial<User> & { password?: string }): Observable<User> {
    this.setLoading(true);
    this.setError(null);
    ...
    return of(null).pipe(
      switchMap(async () => {
        try {
          // CONECTIVIDAD CON API: Actualizar usuario con contraseña si se proporciona
          if (userData.password) {
